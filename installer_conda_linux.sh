...
main() {
    ...
    print_status "Installing Qt/X11 libraries (required for the GUI to display)..."
    ...
    if [ "$HAS_GPU" = true ] && [ "$INSTALL_SEG_GUI" = true ]; then
        ...
        if conda run --no-capture-output -n "$ENV_NAME" python -c "import torch; exit(0 if torch.cuda.is_available() else 1)" > /dev/null 2>&1; then
            ...
            print_warning "Check your NVIDIA driver / CUDA setup."
        fi
    fi
...
EOF
    ...
    create_local_shortcut "ChronoRoot Analysis" "ChronoRootAnalysis" "singlePlantAnalysis" "logo.ico"
