...
def graphInit(graph):
    ...
    # Find main root tip as the endpoint with the longest weighted path from the seed.
    # This is more robust than picking the bottommost node, which fails when a lateral
    # root tip dips below the true main root tip.
    endpoints = [n for n in node_list if n != seed_node and graph.degree(n) == 1]
    if not endpoints:
        endpoints = [n for n in node_list if n != seed_node]

    tip_node = None
    best_length = -1
    for ep in endpoints:
        try:
            length = nx.shortest_path_length(graph, source=seed_node, target=ep, weight='weight')
    ...
    tip_position = np.array(graph.nodes[tip_node]['pos'])
...
def matchGraphs(previous_graph, current_graph, max_movement=70):
    ...
    # Re-derive main root tip as the longest-path endpoint from the seed.
    # Tracking (Phase 2) keeps tip_node close to the previous frame's tip, but
    # the longest-path endpoint is the more reliable structural choice and
    # prevents a lateral that dips below the true tip from stealing the main path.
    curr_nodes_list = list(current_graph.nodes())
    curr_endpoints = [n for n in curr_nodes_list if n != seed_node and current_graph.degree(n) == 1]
    if not curr_endpoints:
        curr_endpoints = [n for n in curr_nodes_list if n != seed_node]

    best_tip = tip_node  # fallback to tracked tip
    best_length = -1
    for ep in curr_endpoints:
        try:
            length = nx.shortest_path_length(current_graph, source=seed_node, target=ep, weight='weight')
            if length > best_length:
                best_length = length
                best_tip = ep
        except nx.NetworkXNoPath:
