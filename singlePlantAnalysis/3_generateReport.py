...
if __name__ == "__main__":
    ...
    if not os.path.exists(temporal_data_path) or FORCE_REPORT:
        for exp_dir in experiments:
            ...
            first_meta = utils.load_paths(exp_dir, '*/*/Results*/metadata.json')
            if first_meta:
                try:
            ...
            camera_paths = utils.load_paths(exp_dir, '*')
            for camera in camera_paths:
                camera_name = os.path.basename(camera)
                plant_paths = utils.load_paths(camera, '*')
                for plant in plant_paths:
                    ...
                    plant_id = f"{camera_name}_{plant_name}"
