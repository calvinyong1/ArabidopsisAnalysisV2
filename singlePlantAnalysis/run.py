...
# Make Homebrew libraries (e.g. libzbar) findable on Apple Silicon
...
class Ui_ChronoRootAnalysis(QtWidgets.QMainWindow):
    ...
    def loadJsonIntoFields(self):
        ...
        try:
            with open(json_path, 'r') as f:
                data = json.load(f)
                for field in [self.plantField, self.processingLimitField,
                      self.processingLimitField_3, self.emergenceDistanceField, self.captureIntervalField,
                      self.everyXhourField, self.everyXhourFieldFourier, self.everyXhourFieldAngles, self.numComponentsFPCAField]:
    ...
    def refresh_table(self):
        ...
        for file in data_files:
            ...
            data.append([experiment, plant, results, error_rate, status, date, file])
    ...
    def setup_field_validation(self):
        ...
        for field in [self.plantField, self.experimentName]:
    ...
    def get_image_paths(self):
        ...
        experiment = self.selected_plant.split(os.path.sep)[-3]
        plant = self.selected_plant.split(os.path.sep)[-2]

        filename = experiment + "_" + plant + ".png"
    ...
    def analysis(self):
        ...
        subprocess.Popen([sys.executable, "1_analysis.py", "--config", os.path.join(self.projectField.text(), "project_config.json")])

    def redoAnalysis(self):
        metadata_path = os.path.join(self.selected_plant, "metadata.json")
        subprocess.Popen([sys.executable, "1_analysis.py", "--config", metadata_path, "--restart"])
    ...
    def repeatAnalysis(self):
        ...
        subprocess.Popen([sys.executable, "1_analysis.py", "--config", metadata_path, "--rerun"])
    ...
    def PostProcess(self):
        self.saveFieldsIntoJson()
        subprocess.Popen([sys.executable, "2_postprocess.py", "--config", os.path.join(self.projectField.text(), "project_config.json")])

    def report(self):
        self.saveFieldsIntoJson()
        subprocess.Popen([sys.executable, "3_generateReport.py", "--config", os.path.join(self.projectField.text(), "project_config.json")])   
    ...
    def setup_tab2_elements(self):
        ...
        self.table.setHorizontalHeaderLabels(["Experiment", "Plant Number", "Result ID",
